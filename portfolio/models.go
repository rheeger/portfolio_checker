...
func OneHolding(r *http.Request) (Holding, error) {
	hldgs := Holding{}
	BinanceTicker := r.FormValue("BinanceTicker")
	if BinanceTicker == "" {
		return hldgs, errors.New("400. Bad Request.")
	}
	err := config.PortfoliosDB.Find(bson.M{"BinanceTicker": BinanceTicker}).One(&hldgs)
	if err != nil {
		return hldgs, err
	}
	return hldgs, nil
...
func PutHolding(r *http.Request) (Holding, error) {
	// get form values
	hldgs := Holding{}
	hldgs.BinanceTicker = r.FormValue("BinanceTicker")
	hldgs.Name = r.FormValue("Name")
	amnt := r.FormValue("Amount")

	// validate form values
	if hldgs.BinanceTicker == "" || hldgs.Name == "" || amnt == "" {
...
func UpdateHolding(r *http.Request) (Holding, error) {
	...
	// update values
	err = config.PortfoliosDB.Update(bson.M{"BinanceTicker": hldgs.BinanceTicker}, &hldgs)
	if err != nil {
		return hldgs, err
	}
	return hldgs, nil
...
func DeleteHolding(r *http.Request) error {
	...
	err := config.PortfoliosDB.Remove(bson.M{"BinanceTicker": BinanceTicker})
	if err != nil {
